...
def extract_using_tesseract(image_path):
    result = {
        "fullname": None,
        "surname": None,
        "middleName": None,
    ...
    for i, line in enumerate(lines):
        if "birth" in line.lower():
            ...
            if i + 1 < len(lines):
                ...
                if next_line:
                    return next_line
    # Fallback: look for line with TAMIL NADU / city
    for line in lines:
...
def extract_place_of_issue(lines):
    # First priority: look for keyword 'issue'
    for i, line in enumerate(lines):
        # print(line, "<---------------------------------- lines")
    ...
    # Second priority: search from bottom for lines with uppercase and <=2 words (likely a city/region)
    for line in reversed(lines):
        cleaned_line = line.strip().replace(":", "")
        if re.match(r"^[A-Z\s]{3,}$", cleaned_line) and len(cleaned_line.split()) <= 2:
            # Ignore MRZ or name lines like 'KARUPPAIYA'
            if not re.match(r'^[A-Z]+<', cleaned_line):  # Not MRZ format
                if not any(keyword in cleaned_line.upper() for keyword in ["SURNAME", "GIVEN NAME", "NATIONALITY", "SEX", "DATE", "AUTHORITY"]):
                    # print(cleaned_line.strip(),"<------------------------------------------------- cleaned_line.strip()")
